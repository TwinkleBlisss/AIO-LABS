...
from cell_formation_problem import CFP
import math
import numpy as np
from copy import deepcopy

class SimulatedAnnealing:
    def __init__(self, matrix, T0, Tf,
                  alpha, max_iterations, D, max_stagnant, trapped_percentage=0.25):
        self.matrix = matrix
        self.T0 = T0
        self.Tf = Tf
        self.alpha = alpha
        ...
        self.D = D
        self.couner = self.counter_mc = self.counter_trapped = self.counter_stagnant = 0
        self.current_solution = None
        self.best_local_solution = None
        self.best_solution = None
        self.trapped_percentage = trapped_percentage

    def __move_helper(split_parts):

        suitable_cells = [index for index, cell in enumerate(split_parts) if len(cell) >= 2]
        cell_index = np.random.randint(0, len(suitable_cells))
        cell = suitable_cells[cell_index]
    ...
    def __iteration(self):
        while self.counter_mc < self.max_iterations and self.counter_trapped < self.max_iterations * self.trapped_percentage:
            ...
            best_local_solution_target = self.cfp.target_function(best_local_solution_target[0], best_local_solution_target[1])
            if new_solution_target > best_local_solution_target:
                ...
                delta_target = new_solution_target - self.cfp.target_function(self.current_solution[0], self.current_solution[1])
